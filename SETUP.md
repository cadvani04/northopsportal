...
- **AI meeting processor** — extracts tasks, deliverables, timeline from transcripts (needs `OPENAI_API_KEY`)
...
4. **Set env vars:**
   ...
   - `OPENAI_API_KEY` (optional, for AI meeting processing)
...
2. Runs AI extraction (if `OPENAI_API_KEY` set) or falls back to action items
...
|---------|---------|---------|
| OpenAI | `OPENAI_API_KEY` | Smart transcript → tasks/deliverables |
